...
def p_losses_hook(x_start, cond, t, noise=None, scale=5.0):
    ...
    img = (x_noisy-extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape)
           * model_output)/extract_into_tensor(self.sqrt_alphas_cumprod, t, x_start.shape)
...
def train_apt_embedding(embedding_name, learn_rate, batch_size, data_root, log_directory, template_file, training_width, training_height, steps, create_image_every, save_embedding_every, save_image_with_stored_embedding,
    ...
    disc = XPDiscriminator(classifier_path) if (
        classifier_path is not None) and os.path.exists(classifier_path) else None
    ...
    for i, entries in pbar:
        ...
        with torch.autocast("cuda"):
            ...
            if use_negative:
                uc = cond_model([entry.cond_text.replace(
                    ds.placeholder_token, ds.placeholder_token+'-uc') for entry in entries])
                c_in = torch.cat([uc, c])
            else:
                c_in = c
            ...
            if disc is not None:
                ...
                loss = output[0] + torch_functional.l1_loss(
                    torch.cat([entry.timg for entry in entries]), x_samples_ddim)*rec_loss_w
            else:
                loss = output[0]
        ...
        if embedding.step > 0 and embedding_dir is not None and embedding.step % save_embedding_every == 0:
            last_saved_file = os.path.join(
                embedding_dir, f'{embedding_name}-{embedding.step}.pt')
            embedding.save(last_saved_file)
            if use_negative:
                last_saved_file = os.path.join(
                    embedding_dir, f'{embedding_name}-uc-{embedding.step}.pt')
        ...
        if embedding.step > 0 and images_dir is not None and embedding.step % create_image_every == 0:
            ...
            if preview_from_txt2img:
                ...
                p.negative_prompt = preview_prompt.replace(
                    ds.placeholder_token, ds.placeholder_token + '-uc') if use_negative else preview_negative_prompt,
