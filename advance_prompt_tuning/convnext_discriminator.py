...
def add_conv_next_path():
    from modules.paths import sd_path
    module_path = os.path.join(sd_path, '../ConvNeXt')
    must_exist_path = os.path.abspath(os.path.join(module_path, 'ConvNeXt/convnext.py'))
...
add_conv_next_path()
import ConvNeXt.convnext
from ConvNeXt.convnext import convnext_tiny as convnext_tiny_raw
...
class XPDiscriminator():
    ...
    def get_score(self, img):
        ...
        img=torch_functional.interpolate(img, size=(self.args.input_size, self.args.input_size), mode='bicubic', align_corners=True)
        pred = self.net(img)
